...
function writeFLAC(
  ...
  interface Block { type: number; data: Buffer }
  const blocks: Block[] = [];
  let off = 4;
  for (;;) {
  ...
  // vendor
  { const len = Buffer.alloc(4); len.writeUInt32LE(vendorBuf.length, 0);
    vcParts.push(len, vendorBuf); }
  // comment count
  { const len = Buffer.alloc(4); len.writeUInt32LE(comments.length, 0);
    vcParts.push(len); }
  // each comment
  for (const c of comments) {
    const cb = Buffer.from(c, 'utf-8');
    const l  = Buffer.alloc(4); l.writeUInt32LE(cb.length, 0);
    vcParts.push(l, cb);
  }
  ...
  if (imageData) {
    ...
    { const b = Buffer.alloc(4); b.writeUInt32BE(3,             0); pp.push(b); } // type: front cover
    { const b = Buffer.alloc(4); b.writeUInt32BE(mimeBuf.length, 0); pp.push(b, mimeBuf); }
    { const b = Buffer.alloc(4); b.writeUInt32BE(0,              0); pp.push(b); } // desc empty
    { const b = Buffer.alloc(4); b.writeUInt32BE(0,              0); pp.push(b); } // width
    { const b = Buffer.alloc(4); b.writeUInt32BE(0,              0); pp.push(b); } // height
    { const b = Buffer.alloc(4); b.writeUInt32BE(0,              0); pp.push(b); } // depth
    { const b = Buffer.alloc(4); b.writeUInt32BE(0,              0); pp.push(b); } // num colors
    { const b = Buffer.alloc(4); b.writeUInt32BE(imageData.length,0); pp.push(b, imageData); }
